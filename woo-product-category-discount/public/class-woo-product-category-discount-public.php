...
class WPCD_Category_Discount_Public {
	...
	public function enqueue_styles() {
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/woo-product-category-discount-public.css', array(), $this->version, 'all' );
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/woo-product-category-discount-public.js', array( 'jquery' ), $this->version, false );
		if( is_cart() ){
			$gifts = array();
			...
			if( !empty( $gifts ) ){
				wp_enqueue_script( $this->plugin_name . '-free-gift-popup', plugin_dir_url( __FILE__ ) . 'js/woo-product-category-discount-public-free-gift-popup.js', array( 'jquery' ), $this->version, false );
				wp_localize_script( $this->plugin_name . '-free-gift-popup', 'wpcd_free_gift_popup', array( 
					...
					'gifts' 	=> $gifts,
					'i18n'  	=> array(
						'close' 			=> __( 'Close', 'woo-product-category-discount' ),
						'add_to_cart' 		=> __( 'Add to cart', 'woo-product-category-discount' ),
						'free_gift_label'	=> __( 'Free gifts', 'woo-product-category-discount' ),
					)
				) );
			}
		}
	...
	public function add_discount_to_cart( $cart ){
		...
		if( $final_discount > 0 ){
			$cart->add_fee( __('Discount', 'woo-product-category-discount'), -$final_discount );
		}
